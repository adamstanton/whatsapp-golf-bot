...
(function(lib, cjs) {
	...
	(lib.ExampleBase = function(
		...
		_this._body = document.getElementById('body');
		_this._target = document.getElementById('target');
		_this._green = document.getElementById('green');	
		
		_this._target.addEventListener('click',function() {
				_this._type = 'TARGET';
				_this._bitmap.visible = false;
				_this._bitmap1.visible = false;
				_this._bitmap2.visible = false;
				_this._bitmap3.visible = false;
				_this._bitmapPlayer.visible = false;
				_this._bitmapTarget.visible = false;
											_this._bitmapGreen.visible = false;
			_this._bitmapGreen1.visible = false;
			_this._bitmapGreen2.visible = false;
			_this._bitmapGreen3.visible = false;
			_this._bitmapGreen4.visible = false;
				_this._brfManager.reset();
		});
		_this._type = 'MULTIPLE';
		_this._body.addEventListener('contextmenu',function(ev) {
				ev.preventDefault();
				_this._bitmap.visible = false;
				_this._bitmap1.visible = false;
				_this._bitmap2.visible = false;
		...
		_this._cameraResolution		= cameraResolution	|| new lib.Rectangle(   0,   0,  960, 540),	// Camera resolution
		_this._brfResolution		= brfResolution		|| new lib.Rectangle(   0,   0,  960, 540),	// BRF BitmapData size
		_this._brfRoi				= brfRoi			|| new lib.Rectangle(  0,  0,  960, 540),	// BRF region of interest within BRF BitmapData size
		...
		_this._screenRect			= screenRect		|| new lib.Rectangle(   0,   0,  960, 540),	// Shown video screen rectangle
...
//		_this._screenRect			= screenRect		|| new lib.Rectangle(   0,   0, 1280, 720),	// Shown video screen rectangle
		...
		_this.initGUI = function() {
			...
			_this._bitmap = new cjs.Bitmap("target.png");
			_this._bitmap1 = new cjs.Bitmap("target.png");
			_this._bitmap2 = new cjs.Bitmap("target.png");
			_this._bitmap3 = new cjs.Bitmap("target.png");
			_this._bitmapPlayer = new cjs.Bitmap("target.png");
			_this._bitmapTarget = new cjs.Bitmap("target.png");
...
//			_this._container.addChild(_this._stats);
			...
			_this._container.addChild(_this._bitmap1);
			_this._container.addChild(_this._bitmap2);
			_this._container.addChild(_this._bitmap3);
			_this._container.addChild(_this._bitmapPlayer);
			_this._container.addChild(_this._bitmapTarget);
			_this._container.addChild(_this._bitmapGreen);
			_this._container.addChild(_this._bitmapGreen1);
			_this._container.addChild(_this._bitmapGreen2);
			_this._container.addChild(_this._bitmapGreen3);
			_this._container.addChild(_this._bitmapGreen4);
		...
		_this.initCamera = function() {
			_this._camera = document.getElementById('videoIn');
		...
		_this.updateMatrices = function() {
			...
			if(_this._mirrored) {
				...
				_this._videoToScreenMatrix.translate(0.0, 0.0);
...
//			_video.smoothing = true;
			...
			if(_this._mirrored) {
				...
				_this._videoToBRFMatrix.translate(0.0, 0.0);
			}
