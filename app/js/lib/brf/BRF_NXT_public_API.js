(function(lib) {
	...
	lib.DrawingUtils.drawLine = function(g, rect, clear, lineThickness, lineColor, lineAlpha) {
		clear = lib.defaultValue(clear, false);
		lineThickness = lib.defaultValue(lineThickness, 1.0); // "#00f6ff"
		lineColor = lib.defaultValue(lineColor, "rgba(0,246,255,1.0)"); // "#00f6ff"
		lineAlpha = lib.defaultValue(lineAlpha, 1.0);
	...
	lib.DrawingUtils.drawPoint = function(g, point, radius, clear, fillColor, fillAlpha, type) {
		console.log ("In DrawPoint");
		clear = lib.defaultValue(clear, false);
		fillColor = lib.defaultValue(fillColor, "rgba(0,246,255,1.0)"); // "#00f6ff"
		fillAlpha = lib.defaultValue(fillAlpha, 1.0);
		var lineThickness = lib.defaultValue(lineThickness, 3); // "#00f6ff"
		var lineColor = lib.defaultValue(lineColor, "rgba(32, 128, 230,0.6)"); // "#00f6ff"
		var lineAlpha = lib.defaultValue(lineAlpha, 0.1);
		clear && g.clear();
		g.beginLinearGradientStroke(["rgba(50, 142, 197, 0.7)", "rgba(50, 142, 197, 1)"], [0,1], point.x, 0, point.x + 5, 0)
		// g.beginFill(fillColor);
		...
		} else if (type === 'MULTIPLE' || type === 'GREEN') {
			g.moveTo(point.x + 3,point.y - 100);
			g.lineTo(point.x,point.y);
			g.moveTo(point.x,point.y - 100);
			g.lineTo(point.x,point.y);
			g.moveTo(point.x - 3,point.y - 100);
			g.lineTo(point.x,point.y);
		}
